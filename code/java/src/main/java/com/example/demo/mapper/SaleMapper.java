...
public interface SaleMapper extends BaseMapper<Sale> {
    @Select("select sa.id,sa.riqi,customer_id,sh_staff,pick,product_id,sa.num,xiaoji,sa.remarks,type,customer,customer_num,area,leibie,salesman,product_name,spec,pinhao,attribute,unit,sa.price,p.pinyin,sa.sale_state,sa.sale_type from (select s.id,s.riqi,customer_id,sh_staff,pick,wuliu_order,product_id,pihao,num,xiaoji,s.remarks,warehouse,type,express,customer,customer_num,area,leibie,salesman,pinyin,fahuo,s.price,sale_state,sale_type,chuku_insert from sale s left join customerInfo c on s.customer_id=c.id) as sa left join product p on sa.product_id=p.id where sa.chuku_insert != '1' order by sa.riqi desc,customer,sale_type")
    List<Sale> getList();
