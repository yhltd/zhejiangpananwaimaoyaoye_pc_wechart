...
public interface kanbanMapper extends BaseMapper<kanban>{
    ...
    @Select("select '出库' as type,s.riqi,isnull(customer,'') as customer,isnull(salesman,'') as salesman,chuku_state as state from sale as s left join customerInfo as c on s.customer_id = c.id where chuku_state = '审核中' group by s.riqi,customer,salesman,chuku_state")
    ...
    @Select("select '出库' as type,s.riqi,isnull(customer,'') as customer,isnull(salesman,'') as salesman,chuku_state as state from sale as s left join customerInfo as c on s.customer_id = c.id where chuku_state = '审核中' and salesman =#{name} group by s.riqi,customer,salesman,chuku_state")
