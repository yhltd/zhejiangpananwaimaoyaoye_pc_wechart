...
public interface RukuMapper extends BaseMapper<Ruku> {
    @Select("select r.id,product_id,riqi,warehouse,staff,product_name,pinhao,spec,attribute,unit,pihao,r.num,remarks,state,product_date,validity from ruku r left join product p " +
            "on r.product_id=p.id ")
    List<Ruku> getList();

    @Select("select r.id,product_id,riqi,warehouse,staff,product_name,pinhao,spec,attribute,unit,pihao,r.num,remarks,state,product_date,validity from ruku r left join product p " +
            "on r.product_id=p.id where riqi=#{riqi} and staff=#{salesman} and state=#{type}")
    ...
    @Select("select r.id,product_id,riqi,warehouse,staff,product_name,pinhao,spec,attribute,unit,pihao,r.num,remarks,state,product_date,validity from ruku r left join product " +
