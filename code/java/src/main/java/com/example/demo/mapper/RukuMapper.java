...
public interface RukuMapper extends BaseMapper<Ruku> {
    @Select("select r.id,product_id,riqi,warehouse,staff,product_name,spec,unit,pihao,r.num,remarks,state from ruku r left join product p " +
            "on r.product_id=p.id ")
    List<Ruku> getList();

    @Select("select r.id,product_id,riqi,warehouse,staff,product_name,spec,unit,pihao,r.num,remarks,state from ruku r left join product " +
